...
$fields->addField('password', 'Must be at least 8 characters, include digit and uppercase.');
...
switch ($action) {
    ...
    case 'register':
        ...
        $validate->text('address', $address, $required = false);
        $validate->text('city', $city, $required = false);
        $validate->state('state', $state, $required = false);
        $validate->zip('zip', $zip, $required = false);
