...
class Validate {
    ...
    public function birthdate($name, $value) {
        ...
        $datePattern = '/^(0?[1-9]|1[012])\/(0?[1-9]|1[0-9]|2[0-9]|3[0-1])\/[1-2][[:digit:]]{3}?$/';
        ...
        $birthdateDate = new DateTime($value);
        $now = new DateTime();
        if ( $birthdateDate > $now ) {
            $field->setErrorMessage('Birthdate must be in the past.');
            return;
        }
        $field->clearErrorMessage();
    }
    ...
    public function email($name, $value, $required = true) {
        ...
        if (filter_var($value, FILTER_VALIDATE_EMAIL) == FALSE) {
            $field->setErrorMessage('There is an error. Please enter again.');
        }


        /* Provided validation for email
        ...
        */
        ...
        if ($pwMatch === false) {
            ...
            $field->setErrorMessage(
                    'Must use at least one uppercase and digit and include at least 8 characters');
            return;
        }
    }
